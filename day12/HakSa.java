...
//====== 2. 레이블과 입력창 객체를 생성한다.=================================================================								
		...
		this.tfID = new JTextField(14); 		// 14 : 컬럼 사이즈 (글자수)
		this.btnSearch = new JButton("검색");	// 학번 검색할 버튼 생성
...
//		this.add(new JScrollPane(this.taList)); // 텍스트에어리어에 스크롤을 적용
		...
		table.setPreferredScrollableViewportSize(new Dimension(250, 290));
		...
		this.btnInsert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					// insert문
					st.executeUpdate("INSERT INTO student VALUES('" + tfID.getText() + "', '"+ tfName.getText() +"', '"+ tfDepartment.getText() +"');");
					// 텍스트에어리어 초기화
					tfID.setText("");
					tfName.setText("");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(result == JOptionPane.YES_OPTION) { 
					...
					try {
						...
						// update문
						st.executeUpdate("UPDATE student SET name = '"+ tfName.getText() +"', dept = '"
